...
def describe(image_path, cropped_image_path, cont=True, bright=True, haze=True, sharpness=True, color_hist=True,
             intensity_hist=True, white_thresh=175):
    image = cv2.imread(image_path, cv2.IMREAD_COLOR)
    cropped_image_rgb = cv2.imread(cropped_image_path, cv2.IMREAD_COLOR)

    if cropped_image_rgb is None:
        print(f"Error: Unable to read image from {cropped_image_path}")
        with open("error_log.txt", "a") as log_file:
            log_file.write(f"Error: Unable to read image from {cropped_image_path}\n")
        return np.zeros(30000)
...
def create_features(standart_image_dir, cropped_image_dir, dest, cont=True, bright=True, haze=True, sharpness=True,
                    color_hist=True, intensity_hist=True, white_thresh=175):
    img_features = []
    img_labels = []
    batch_counter = 0
    fc = 1
    ...
    for dir_name in dirs:
        ...
        for image_name in class_images:
            ...
            if image_name[-4:] == '.jpg':  # if image extension is jpg
                ...
                img_feature = describe(std_img_path, cropped_img_path, cont, bright, haze, sharpness, color_hist,
                                       intensity_hist, white_thresh)
